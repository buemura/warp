...
async function getFileInfo(shortId: string): Promise<FileInfoResponse> {
  const res = await fetch(`/api/files/${shortId}`);
...
async function accessFile(shortId: string, password?: string): Promise<Blob> {
  const res = await fetch(`/api/files/${shortId}/access`, {
    method: "POST",
...
function StatusCard({
  ...
  return (
    ...
    >
      ...
      <Link href="/" className="warp-link" style={{ fontSize: "14px" }}>
        ← Back to Warp
      </Link>
    </div>
  );
